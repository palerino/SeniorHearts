...
if (scr_isNear(objPlayer)) {	
	...
	if(keyboard_check_pressed(vk_space)) {
		...
		if(itemTextbox == noone) {
			...
			if(!objGameManager.paintingSupplies) { //can be picked up due to a relevant event
	...
	if(itemTextbox != noone) {
		...
		if(!objGameManager.paintingSupplies) {
			objGameManager.scruffyPaintingSupplies = true;
			instance_destroy();
		}
	}
}
